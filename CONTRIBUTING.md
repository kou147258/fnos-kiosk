...
欢迎加入 fnos-kiosk 开发！本项目沿用 fnos-dashboard 的「零第三方 Python 依赖 +
...
- `fb_render.py` —— fb 渲染器 + `BrowserCanvas`（截图解码 / 缩放 / 旋转）。
...
- [ ] `python3 app/bin/dashboard.py --port 8200 --web app/web` 能启动
- [ ] `curl http://127.0.0.1:8200/api/status` 返回 ok
- [ ] `curl -X POST http://127.0.0.1:8200/api/settings -d '{}' -H 'Content-Type: application/json'`
      从非本机返回 403（本机可用）
- [ ] fb 路径写 `/dev/fb0` 不存在时仍能拉起 dashboard，仅 fb 渲染进程不启动
...
- `/var/apps/com.fnos.kiosk/var/app.log` 与 `fb_chromium.log` 相关片段

## 🔁 PR 流程

1. fork → 新建分支（`feature/<name>` 或 `fix/<name>`）
2. 本地通过自检清单
