...
# fnOS 浏览器屏 —— 构建与打包脚本
# 用法：./build.sh
#       （fnpack 优先；不在 PATH 时用 tools/pack_fpk.py 兜底打包）
set -e
cd "$(dirname "$0")"
...
echo ""
echo "本地预览（无需 fnOS）：python3 app/bin/dashboard.py --port 8200 --web app/web"
echo "设置页：http://127.0.0.1:8200/settings（仅限本机）"